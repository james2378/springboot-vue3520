<template>
	<div class="diy_details_box diy_div_city_classification">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "city_classification_id",
					source_id: this.obj["city_classification_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/city_classification/set?city_classification_id=' + _this.obj["city_classification_id"], {
								praise_len
							}, (res) => {
								if(res.result){
