		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/school_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_school_information = !show_list_school_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_school_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--学校信息推荐列表组件 -->
		                <list_school_information v-if="show_list_school_information" :list="list_school_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_school_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_school_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_school_information" :key="i" @click="to_details('school_information',o,'school_information_id')">
		                            <td v-for="(oj,n) in fields_school_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_school_information: true,
				list_school_information: [],
				vm_arr_school_information: [
					...
					"founding_year_",
					"school_pictures",
					"enrollment_score_line",
					"enrollment_time",
					"consultation_telephone",
					"school_address",
					"school_introduction",
				],
				fields_school_information: [
					{
						key: "school_name",
						label: "学校名称",
						type: "文本"
		...
		methods: {
			...
			get_school_information() {
				let url = "~/api/school_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_school_information = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_school_information() {
		        var list = this.list_school_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_school_information.map((o) => {
		                // 第二个中括号是对象的属性
