<template>
	<div class="diy_details page_school_information" id="school_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_school_information'>
							<div_school_information :obj="obj"></div_school_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_school_information from "@/components/diy/div_school_information.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_school_information
		},
		data(){
			return{
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/school_information/set?school_information_id=' + obj["school_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							...
							source_field: "school_information_id",
							source_id: this.obj.school_information_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "school_information",
					source_field: "school_information_id",
					source_id: obj["school_information_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.school_name
					query.img = this.obj.school_pictures
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/school_information/set?school_information_id=' + _this.obj["school_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
